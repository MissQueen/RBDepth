...
class retinexltifpmGenerator(nn.Module):
    def __init__(self, opt=None):
        ...
        self.reflectance_dec = ContentDecoder(opt.n_downsample, r_dec_n_res,
                                              self.reflectance_enc.output_dim, opt.output_nc, opt.ngf, 'ln', opt.activ, pad_type=opt.pad_type)

        self.illumination_enc = ContentEncoder(opt.n_downsample, i_enc_n_res,
                                               opt.input_nc, self.reflectance_dim, opt.ngf, 'in', opt.activ, pad_type=opt.pad_type)
        self.illumination_dec = ContentDecoder(opt.n_downsample, i_dec_n_res,
                                               self.illumination_enc.output_dim, opt.output_nc, opt.ngf, 'ln', opt.activ, pad_type=opt.pad_type)
    ...
    def forward(self, src_img):
        ...
        r_content = self.reflectance_enc(src_img)
        i_content = self.illumination_enc(src_img)
