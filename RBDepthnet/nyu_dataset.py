...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None):
        ...
        self.filenames = []
        if mode == 'train':
            with open(os.path.join(args.data_path, 'train.csv')) as f:
                for line in f:
                    self.filenames.append(line)
        else:
            with open(os.path.join(args.data_path, 'test.csv')) as f:
                for line in f:
                    self.filenames.append(line)
        self.mode = mode
        transform_list = [
    ...
    def __getitem__(self, item):
        ...
        image_path = os.path.join(self.args.data_path, sample_path.strip().split(',')[0])
        depth_path = os.path.join(self.args.data_path, sample_path.strip().split(',')[1])
